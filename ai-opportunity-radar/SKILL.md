...
3. **Who scores** — the user/org gives value/feasibility & readiness, or I put down **estimates marked 待調** for them to adjust? (Never present estimates as the org's real scores.)

## Phase 1 — assemble the portfolio (apply the assessment lenses)

Build one `portfolio.json`:
- `use_cases[]`: `{name, capability (a BCM L1 box), value 1-5, feasibility 1-5, type: defend|extend|upend}`. When proposing/sizing each, apply note 1's lenses (documented in `bcm-使用與治理.md`):
...
- `notes[]`: flag every estimate (`待調`), the scoring wall, and the type-inference basis. Worked example: `references/example-portfolio.json`.
...
- **Don't present estimated scores as the org's real numbers.** value/feasibility/readiness are internal strategic judgements; pre-fill as `待調` estimates and say so. A radar full of made-up scores is worse than an empty one.
...
- This skill is the *use* step; making the map itself is [[bcm-creator]], and the connecting method is `bcm/bcm-使用與治理.md`.
